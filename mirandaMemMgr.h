...
class MirandaMemoryManager {
  ...
  public:
    MirandaMemoryManager(SST::Output *mgrOutput, const uint64_t _pageSize, const uint64_t _pageCount,
                         const MirandaPageMappingPolicy mapPolicy, SharedRegion *mapRegion)
        ...
        if (mapRegion->getLocalShareID() == 0) { // First sharer, we're in charge
            ...
            case RANDOMIZED:
                output->verbose(CALL_INFO, 2, 0,
                                "Memory is set to RANDOMIZED mapping, will perform a randomized shuffle of pages...\n");
                ...
                for (uint64_t i = 0; i < (pageCount * 2); ++i) {
                    ...
                    if (selectA != selectB) {
                        ...
                        output->verbose(CALL_INFO, 64, 0,
                                        "Swapping index %" PRIu64 " with index %" PRIu64 ", pageA=%" PRIu64
                                        ", pageB=%" PRIu64 "\n",
    ...
    void setSharedAddressMap(const uint64_t *addrmap) { pageMap = addrmap; }
...
#define MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL 16
        ...
        if (__builtin_expect(output->getVerboseLevel() >= MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0)) {
            output->verbose(CALL_INFO, MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0,
                            "Mapping v-addr: %" PRIu64 " pageOffset: %" PRIu64 " -> (%" PRIu64 " - %" PRIu64
                            ")/%" PRIu64 " = v-page-index: %" PRIu64 ", p-start: %" PRIu64 " + poffset %" PRIu64
                            " = %" PRIu64 "\n",
                            addrIn, pageOffset, addrIn, pageOffset, pageSize, virtualPageStart, physPageStart,
                            pageOffset, physAddress);
        }
