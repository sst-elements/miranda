...
class MirandaMemoryManager {
   public:
    MirandaMemoryManager(SST::Output *mgrOutput, const uint64_t _pageSize,
                         const uint64_t _pageCount, const MirandaPageMappingPolicy mapPolicy)
        : pageSize(_pageSize),
          pageCount(_pageCount),
          maxMemoryAddress(_pageSize * _pageCount),
          output(mgrOutput) {
        output->verbose(CALL_INFO, 2, 0,
                        "Creating memory manager, page size=%" PRIu64 ", page count=%" PRIu64
                        ", max address=%" PRIu64 "\n",
        ...
        switch (mapPolicy) {
            case LINEAR:
                output->verbose(
                    CALL_INFO, 2, 0,
                    "Memory is set to LINEAR mapping, will not adjust current page maps\n");
                // Nothing to do
                break;

            case RANDOMIZED:
                output->verbose(CALL_INFO, 2, 0,
                                "Memory is set to RANDOMIZED mapping, will perform a randomized "
                                "shuffle of pages...\n");
                ...
                for (uint64_t i = 0; i < pageCount; ++i) {
                    output->verbose(CALL_INFO, 32, 0,
                                    "Virtual Start = %20" PRIu64 " Physical Start = %20" PRIu64
                                    "\n",
    ...
    auto mapAddress(const uint64_t addrIn) const -> uint64_t {
        if (__builtin_expect(static_cast<long>(addrIn >= maxMemoryAddress), 0) != 0) {
            output->fatal(CALL_INFO, -1,
                          "Error: address %" PRIu64
                          " exceeds the maximum address in Miranda calculated by pageSize (%" PRIu64
                          ") * pageCount (%" PRIu64 ") = %" PRIu64 "\n",
...
#define MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL 16
        ...
        if (__builtin_expect(
                static_cast<long>(output->getVerboseLevel() >= MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL),
                0) != 0) {
            output->verbose(CALL_INFO, MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0,
                            "Mapping v-addr: %" PRIu64 " pageOffset: %" PRIu64 " -> (%" PRIu64
                            " - %" PRIu64 ")/%" PRIu64 " = v-page-index: %" PRIu64
                            ", p-start: %" PRIu64 " + poffset %" PRIu64 " = %" PRIu64 "\n",
                            addrIn, pageOffset, addrIn, pageOffset, pageSize, virtualPageStart,
                            physPageStart, pageOffset, physAddress);
        }
