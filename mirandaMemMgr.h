            ...
            uint64_t mapAddress(const uint64_t addrIn) const {
                if (__builtin_expect(addrIn >= maxMemoryAddress, 0)) {
                    output->fatal(CALL_INFO, -1,
                                  "Error: address %" PRIu64 " exceeds the maximum address in Miranda calculated by pageSize (%" PRIu64 ") * pageCount (%" PRIu64 ") = %" PRIu64 "\n",
...
#define MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL 16
                ...
                if (__builtin_expect(output->getVerboseLevel() >= MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0)) {
                    output->verbose(CALL_INFO, MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0,
                                    "Mapping v-addr: %" PRIu64 " pageOffset: %" PRIu64 " -> (%" PRIu64 " - %" PRIu64 ")/%" PRIu64 " = v-page-index: %" PRIu64 ", p-start: %" PRIu64 " + poffset %" PRIu64 " = %" PRIu64 "\n",
