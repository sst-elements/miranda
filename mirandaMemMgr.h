    ...
    namespace Miranda {
        ...
        public:
            MirandaMemoryManager(
                ...
                output->verbose(CALL_INFO, 2, 0, "Creating memory manager, page size=%"
                PRIu64
                ", page count=%"
                PRIu64
                ", max address=%"
                    ...
                    case RANDOMIZED:
                        ...
                        for (uint64_t i = 0; i < (pageCount * 2); ++i) {
                            ...
                            if (selectA != selectB) {
                                ...
                                output->verbose(CALL_INFO, 64, 0, "Swapping index %"
                                PRIu64
                                " with index %"
                                PRIu64
                                ", pageA=%"
                        ...
                        for (uint64_t i = 0; i < pageCount; ++i) {
                            output->verbose(CALL_INFO, 32, 0, "Virtual Start = %20"
                            PRIu64
                            " Physical Start = %20"
                            PRIu64
                            "\n",
            ...
            uint64_t mapAddress(const uint64_t addrIn) const {
                if (__builtin_expect(addrIn >= maxMemoryAddress, 0)) {
                    output->fatal(CALL_INFO, -1, "Error: address %"
                    PRIu64
                    " exceeds the maximum address in Miranda calculated by pageSize (%"
                    PRIu64
                    ") * pageCount (%"
                    PRIu64
                    ") = %"
...
#define MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL 16
                ...
                if (__builtin_expect(output->getVerboseLevel() >= MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL,
                                     0)) {
