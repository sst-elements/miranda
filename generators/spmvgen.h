...
class SPMVGenerator : public RequestGenerator {
   public:
    ...
    SPMVGenerator(ComponentId_t id, Params &params) : RequestGenerator(id, params) {
        build(params);
    }
    ...
    void build(Params &params) {
        ...
        matrixRowIndicesStartAddr =
            params.find<uint64_t>("matrix_row_indices_start_addr", nextStartAddr);
        nextStartAddr += (ordinalWidth * (matrixNy + 1));

        matrixColumnIndicesStartAddr =
            params.find<uint64_t>("matrix_col_indices_start_addr", nextStartAddr);
    ...
    void generate(MirandaRequestQueue<GeneratorRequest *> *q) override {
        for (uint64_t row = localRowStart; row < localRowEnd; row++) {
            ...
            auto readStart = new MemoryOpRequest(matrixRowIndicesStartAddr + (ordinalWidth * row),
                                                 ordinalWidth, READ);
            ...
            auto readResultCurrentValue =
                new MemoryOpRequest(rhsVecStartAddr + (row * matrixNNZPerRow), elementWidth, WRITE);
            auto writeResult =
                new MemoryOpRequest(rhsVecStartAddr + (row * matrixNNZPerRow), elementWidth, WRITE);
            ...
            for (uint64_t col = row; col < (row + matrixNNZPerRow); col++) {
                ...
                out->verbose(CALL_INFO, 4, 0,
                             "Generating access for row %" PRIu64 ", column: %" PRIu64 "\n", row,
                             col);

                auto readMatElement = new MemoryOpRequest(
                    matrixElementsStartAddr + (row * matrixNNZPerRow + col) * elementWidth,
                    elementWidth, READ);
                auto readCol = new MemoryOpRequest(
                    matrixColumnIndicesStartAddr + (row * matrixNNZPerRow + col) * ordinalWidth,
                    ordinalWidth, READ);
                auto readLHSElem = new MemoryOpRequest(
                    lhsVecStartAddr + (row * matrixNNZPerRow + col) * elementWidth, elementWidth,
                    READ);
    ...
    auto isFinished() -> bool override { return (0 == iterations); }

    void completed() override {}

    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(SPMVGenerator, "miranda", "SPMVGenerator",
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"ordinal_width", "Sets the width of ordinals (indices) in the matrix, typically 4 or 8",
         "8"},
