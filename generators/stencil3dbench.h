...
class Stencil3DBenchGenerator : public RequestGenerator {
   public:
    ...
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(Stencil3DBenchGenerator, "miranda",
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"datawidth", "Sets the data width of the mesh element, typically 8 bytes for a double",
         "8"},
        {"startz",
         "Sets the start location in Z-plane for this instance, parallelism implemented as Z-plane "
         "decomposition",
         "0"},
        {"endz",
         "Sets the end location in Z-plane for this instance, parallelism implemented as Z-plane "
         "decomposition",
         "10"},
        {"iterations", "Sets the number of iterations to perform over this mesh", "1"})

   private:
    void convertIndexToPosition(uint32_t index, uint32_t *posX, uint32_t *posY, uint32_t *posZ);

    auto convertPositionToIndex(uint32_t posX, uint32_t posY, uint32_t posZ) -> uint32_t;
