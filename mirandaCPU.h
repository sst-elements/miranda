...
class RequestGenCPU : public SST::Component {
  public:
    ...
    SST_ELI_DOCUMENT_PARAMS(
        {"max_reqs_cycle", "Maximum number of requests the CPU can issue per cycle (this is for all reads and writes)",
         "2"},
        {"max_reorder_lookups", "Maximum number of operations the CPU is allowed to lookup for memory reorder", "16"},
        {"cache_line_size", "The size of the cache line that this prefetcher is attached to, default is 64-bytes",
         "64"},
        ...
        {"pagemap",
         "Mapping scheme, string set to LINEAR or RANDOMIZED, default is LINEAR (virtual==physical), RANDOMIZED "
         "randomly shuffles virtual to physical map.",
         "LINEAR"},
    ...
    SST_ELI_DOCUMENT_PORTS({"cache_link", "Link to Memory Controller", {"memHierarchy.memEvent", ""}},
    ...
    SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS({"generator", "What address generator to load",
                                        ...
                                        {"memory", "The memory interface to use (e.g., interface to caches)",
